...
import { ArrowLeft, ChevronDown, ChevronRight, Copy, ScanLine, Upload, Wand2, X } from 'lucide-react';
...
export function DeviceForm({ deviceId, onClose }: DeviceFormProps) {
  ...
  const canProvisionWithoutClaim = ['Owner', 'Admin', 'Engineer', 'Operator'].includes(currentUser?.role || '');
  ...
  const [advancedOpen, setAdvancedOpen] = useState(Boolean(existingDevice));
  const provisionIdentityRef = useRef<HTMLInputElement | null>(null);
  const matchedManufacturedDevice = matchedManufacturedId
    ? manufacturedDevices.find((item) => item.id === matchedManufacturedId)
    : null;
  ...
  const applyProvisioningIdentity = (identityValue = provisionIdentity) => {
    ...
    if (!identity) {
      setProvisionMessage('Scan or enter MAC / IMEI / Serial Number first.');
      return;
    }
    ...
    const needsClaimCode = Boolean(manufacturedDevice.claimCode) && !canProvisionWithoutClaim;
    if ((needsClaimCode || provisionClaimCode.trim()) && !isClaimCodeValid(manufacturedDevice.claimCode, provisionClaimCode)) {
      setMatchedManufacturedId('');
      setProvisionMessage(needsClaimCode
        ? 'Invalid claim code. Enter the Claim Code from the manufactured device inventory.'
        : 'Invalid claim code. Leave it empty or enter the correct code.');
...
mqtt pub -h <broker-host> -p 1883 -t "${getMqttTelemetryTopic()}" -m '${JSON.stringify(buildTelemetryPayload())}'`;
  ...
  const handleSave = () => {
    const deviceId = existingDevice?.id || formData.id || `DEV-${Math.random().toString(36).substr(2, 6).toUpperCase()}`;
      ...
      <div className="px-4 py-5 sm:p-6 space-y-6">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
            <div className="flex-1">
              <label className="flex items-center gap-2 text-sm font-semibold text-slate-800 dark:text-slate-100">
                ...
                Quick Bind by Scan / MAC / IMEI / Serial Number
              </label>
              <p className="mt-1 text-xs text-slate-600 dark:text-slate-300">
                Scan a label with a barcode scanner, or manually enter the device identity. The system will fill model, data source, topic/API path, mappings, controls, and icon automatically.
              </p>
              <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div>
                  ...
                  <div className="mt-1 flex rounded-md shadow-sm">
                    <button
                      type="button"
                      onClick={() => provisionIdentityRef.current?.focus()}
                      className="inline-flex items-center gap-1 rounded-l-md border border-r-0 border-slate-300 bg-white px-3 text-xs font-semibold text-orange-700 hover:bg-orange-50 dark:border-slate-700 dark:bg-slate-900 dark:text-orange-300 dark:hover:bg-orange-500/10"
                      title="Focus this field before scanning a QR/barcode label"
                    >
                      <ScanLine className="h-3.5 w-3.5" />
                      Scan
                    </button>
                    <input
                      ref={provisionIdentityRef}
                      ...
                      className="block w-full rounded-r-md border-slate-300 bg-white text-sm text-slate-900 shadow-sm focus:border-orange-500 focus:ring-orange-500 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
                    />
                  </div>
                  <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">Most scanner devices paste the scanned value into the focused field and press Enter.</p>
                </div>
                <div>
                  <label className="text-xs font-semibold uppercase tracking-wider text-slate-500">
                    Claim Code {canProvisionWithoutClaim ? '(optional)' : '(required when configured)'}
                  </label>
                  <input
                  ...
                  <p className="mt-1 text-[11px] text-slate-500 dark:text-slate-400">
                    {canProvisionWithoutClaim ? 'Privileged dashboard users can auto-configure inventory devices without a claim code.' : 'Ask the administrator for the claim code printed on the device label.'}
                  </p>
                </div>
              </div>
            </div>
            <button
              ...
              className="inline-flex h-10 items-center justify-center gap-2 rounded-md bg-orange-600 px-4 text-sm font-semibold text-white hover:bg-orange-500"
            >
              <Wand2 className="h-4 w-4" />
              Auto Configure
            </button>
          </div>
          {matchedManufacturedDevice && matchedDeviceModel && (
            <div className="mt-3 grid gap-3 rounded border border-orange-200 bg-white p-3 text-xs dark:border-orange-500/30 dark:bg-slate-950 sm:grid-cols-4">
              <div>
                <p className="font-semibold uppercase tracking-wider text-slate-500">Model</p>
                <p className="mt-1 text-slate-900 dark:text-slate-100">{matchedDeviceModel.name}</p>
              </div>
              <div>
                <p className="font-semibold uppercase tracking-wider text-slate-500">Type</p>
                <p className="mt-1 text-slate-900 dark:text-slate-100">{formatDeviceTypeLabel(matchedDeviceModel.deviceType, typesT as Record<string, string>)}</p>
              </div>
              <div>
                <p className="font-semibold uppercase tracking-wider text-slate-500">Data Source</p>
                <p className="mt-1 text-slate-900 dark:text-slate-100">{matchedDeviceModel.dataSource.toUpperCase()} / {matchedDeviceModel.protocol || '-'}</p>
              </div>
              <div>
                <p className="font-semibold uppercase tracking-wider text-slate-500">Inventory</p>
                <p className="mt-1 font-mono text-slate-900 dark:text-slate-100">{matchedManufacturedDevice.serialNumber || matchedManufacturedDevice.mac || matchedManufacturedDevice.imei}</p>
              </div>
            </div>
          )}
          {provisionMessage && (
        ...
        <div className="border-t border-slate-200 pt-5 dark:border-slate-800">
          <button
            ...
            className="flex w-full items-center justify-between rounded-lg border border-slate-200 bg-slate-50 px-4 py-3 text-left text-sm font-semibold text-slate-800 transition hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900/40 dark:text-slate-100 dark:hover:bg-slate-900"
          >
            <span>
              Advanced Settings
              <span className="ml-2 text-xs font-normal text-slate-500 dark:text-slate-400">
                Data source, protocol, SCADA icon, templates, and test payloads
              </span>
            </span>
            {advancedOpen ? <ChevronDown className="h-4 w-4 text-slate-500" /> : <ChevronRight className="h-4 w-4 text-slate-500" />}
          </button>
        </div>
