...
import type { AppProfile, ControlAccessConfig, FeatureAccessMap } from './featureAccess';
...
export const useAppStore = create<AppState>()(
    (set) => ({
      ...
      regenerateManufacturedDeviceClaimCode: (manufacturedId, claimCode, user) => set((state) => {
        const manufacturedDevice = state.manufacturedDevices.find((item) => item.id === manufacturedId);
        const log: ProvisioningAuditLog = {
          id: `provision-log-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
          manufacturedDeviceId: manufacturedId,
          identity: manufacturedDevice?.serialNumber || manufacturedId,
